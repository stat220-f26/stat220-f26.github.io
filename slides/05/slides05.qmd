...
`geom_map()` matches each row's `map_id` to a region in a separate `map` data set, so you don't have to track `long`/`lat`/`group` yourself. You *do* need `expand_limits()`, or ggplot won't know the x/y range to show
...
`coord_map()` gives a nicer projection than the default, and `theme_map()` removes the axes
...
::: {.task .nonincremental}
-   Edit this code so that each state is colored in with a different color.
-   Hint: `states` already has a `region` column — you can map it to `fill` the same way it's mapped to `map_id`
