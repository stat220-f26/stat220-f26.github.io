...
plot1 = ggplot(college_pay) + 
  geom_point(aes(x = in_state_tuition, y = mid_career_pay, color = state)) + 
...
::: {.task .nonincremental}
1.  Log into maize
    -   If you have to type your PAT in everytime you push to GitHub, follow the directions at [Getting Set up with Git and GitHub](https://stat220-f26.github.io/computing/git-stat220.html#getting-setup-with-git-and-github) #4 to tell RStudio to save your credentials (may or may not work on maize 🤷‍♀️ but definitely works on local installations)
2.  Find the `.qmd` template for today at the course website
3.  Open it up in maize/Rstudio
4.  Work with a neighbor to recreate this graph &rarr;
:::
::::
...
p_pay = ggplot(college_pay) + 
  geom_point(
    aes(x = in_state_tuition, y = mid_career_pay, color = group), 
    alpha = 0.6,
    size = 2
    ) + 
  scale_color_manual(values = c("gold2", "navyblue", "gray70"))
...
pay_ratio = college_pay |>
  mutate(pay_per_tuition = mid_career_pay / in_state_tuition)
carleton = filter(pay_ratio, name == "Carleton College")

story_base = ggplot(pay_ratio, aes(x = in_state_tuition, color = group)) +
  scale_color_manual(values = c("gold2", "navyblue", "gray70")) +
  scale_x_continuous(labels = scales::dollar) +
...
story1 = story_base +
  geom_point(aes(y = mid_career_pay), alpha = 0.6, size = 2) +
  scale_y_continuous(labels = scales::dollar) +
  annotate("point", x = carleton$in_state_tuition, y = carleton$mid_career_pay,
           shape = 1, size = 7, stroke = 1.2, col = "gold4") +
  labs(x = "In-state tuition", y = "Median mid-career pay",
       title = "Carleton: top 13% in pay")

story2 = story_base +
  geom_point(aes(y = pay_per_tuition), alpha = 0.6, size = 2) +
  annotate("point", x = carleton$in_state_tuition, y = carleton$pay_per_tuition,
           shape = 1, size = 7, stroke = 1.2, col = "gold4") +
  labs(x = "In-state tuition", y = "Mid-career pay per tuition dollar",
       title = "Carleton: bottom 6% in pay per tuition dollar")
...
4. Post to the "Day04" post on our Ed board when you're done (you don't have to post your explanation)

5. Look through the posts and &hearts; any that are especially good (bad)!
:::
